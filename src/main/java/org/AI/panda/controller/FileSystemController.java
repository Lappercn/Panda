...
public class FileSystemController {
    ...
    public Result<java.util.Map<String, String>> preview(@RequestHeader(value = "X-User-ID", required = false) String userId,
        ...
        java.util.Map<String, String> data = fileSystemService.getPreviewData(UserIdResolver.resolve(request, userId), nodeId, includeFileUrl);
        
        // 如果当前是分享访问，且 URL 中包含 shareToken，则将其附加到 ocrUrl 后面
        // 这样前端加载 OCR 数据时带上 Token，避免 403
        String shareToken = request.getParameter("shareToken");
        if (shareToken != null && !shareToken.isBlank() && data.containsKey("ocrUrl")) {
            String ocrUrl = data.get("ocrUrl");
            if (ocrUrl.contains("?")) {
                ocrUrl += "&shareToken=" + shareToken;
            } else {
                ocrUrl += "?shareToken=" + shareToken;
            }
            data.put("ocrUrl", ocrUrl);
        }
    ...
    public ResponseEntity<byte[]> ocr(@RequestHeader(value = "X-User-ID", required = false) String userId,
        ...
        try {
            ...
            // 记录详细的权限拒绝日志
            System.err.println("OCR Access Denied: nodeId=" + nodeId + 
                             ", resolvedUid=" + uid + 
                             ", error=" + e.getMessage());
            // 403 Access denied
